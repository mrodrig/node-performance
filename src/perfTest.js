...
let perf = {
    ...
    makeRequest: function (indx) {
        return function (cb) {
            let start = new Date().getTime(); // Retrieve the time (ms) before starting
            let opts = {
            ...
            request(opts, function (err, res, body) {
                let finished = new Date().getTime(); // Retrieve the time (ms) after receiving the response
    ...
    makePass: function (indx) {
        return function (cb) {
            let requests = perf.createList(perf.OPTIONS.NUM_REQUESTS, perf.makeRequest); // Retrieve the list of requests to async hit
            console.log('\n\nStarting pass #' + indx);
            async.parallel(requests, function (err, res) { // Asynchronously make the requests
                let data = { 'sum': _.reduce(res, function(acc, pass) {return acc + pass.total; }, 0) }; // Compute the total time taken to fulfill the requests
    ...
    multiRequests: function (cb) {
        let passes = perf.createList(perf.OPTIONS.NUM_PASSES, perf.makePass);
